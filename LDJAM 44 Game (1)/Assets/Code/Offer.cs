...
public class Offer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Change4 = "Give";
            WealthValue = stats.WealthValue/10;
        }
        else if (wealthOffer.value < 0)
        {
            Change4 = "Take";
            WealthValue = stats.WealthValue/10 * 2;
        }
        else
        {
            Change4 = "";
    ...
    public void Onclick()
    {
        ...
        totalValue = (EQValue*(int)EQOffer.value  + IQValue*(int)IQOffer.value + AgeValue* (int)ageOffer.value + WealthValue*(int)wealthOffer.value + HappinessValue*(int)happinessOffer.value);
        ...
        if (totalValue >= threshold)
        {
            ...
            IQChange += IQOffer.value/10;
            EQChange += EQOffer.value/10;
            ageChange += ageOffer.value/10;
            wealthChange += wealthOffer.value/10;
            happinessChange += happinessOffer.value/10;
        }
        else
        {
            if (Eventmanager.currentPersonName == "Robber")
